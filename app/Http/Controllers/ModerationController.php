    ...
    try {
        ...
        $achievements = $user->achievements()
            ...
            ->map(function($achievement) use ($user) {
                // Загружаем данные пользователя, который выдал награду
                $awardedBy = $achievement->pivot->awarded_by;
                $awardedByUser = null;

                if ($awardedBy) {
                    $awardedByUser = \App\Models\User::find($awardedBy);
                }
                ...
                return [
                    ...
                    'pivot' => [
                        ...
                        'awarded_by' => $achievement->pivot->awarded_by,
                    ],
                    'awarded_by_user' => $awardedByUser ? [
                        'id' => $awardedByUser->id,
                        'username' => $awardedByUser->username,
                        'name' => $awardedByUser->name,
                    ] : null
                ];
            });
