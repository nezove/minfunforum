...
if ($request->hasFile('files')) {
    foreach ($request->file('files') as $file) {
        if ($file->isValid()) {
            ...
            // AJAX ответ
            if ($request->ajax()) {
                // Загружаем пост с пользователем для ответа
                $post->load('user');
    ...
    'posts_count' => $post->user->posts_count ?? 0,
                        ...
                        'reply_to_post' => $post->reply_to_post_id ? [
    'id' => $post->replyToPost->id ?? null,
    'user_name' => $post->replyToPost->user->name ?? $post->replyToPost->user->username ?? null,
    'content' => $post->replyToPost ? \Illuminate\Support\Str::limit(strip_tags($post->replyToPost->content), 200) : null,
] : null,
